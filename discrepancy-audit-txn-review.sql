/***** 
		Specific station and medication for determination of discrepancy origin
*****/
...
DECLARE @ADC char(6);
DECLARE @ADCunit varchar(8);
DECLARE @ddId int;

	/* specify before execution */
SET @nmDays = 90;		--lookback
SET @ADC = 'tower3';	--station ID
SET @ADCunit = '';		--comment out station conditional in where clause of main query if this is used
SET @ddId = 138824;		--drug dose id

	/** Search START time **/
/*BoD, variable days ago*/ 	SET @searchStartTime = DATEADD(dd,DATEDIFF(dd,0,GETDATE())-@nmDays,0);
/*BoW, last*/ 				--SET @searchStartTime = DATEADD(dd,-1,DATEADD(wk,DATEDIFF(wk,0,GETDATE())-1,0));
/*BoW, current*/ 			--SET @searchStartTime = DATEADD(dd,-1,DATEADD(wk,DATEDIFF(wk,0,GETDATE()),0));
/*Specific*/				--SET @searchStartTime = '2019-02-01 00:00:00.000';

	/** Search END time **/
/*Now*/ 		--SET @searchStopTime = GETDATE();
/*BoD, today*/ 	SET @searchStopTime = DATEADD(dd,DATEDIFF(dd,0,GETDATE()),0);
/*EoW, last*/ 	--SET @searchStopTime = DATEADD(dd,-1,DATEADD(wk,DATEDIFF(wk,0,GETDATE()),0));
/*Specific*/ 	--SET @searchStopTime = '2019-02-07 07:00:00.000';
...
SELECT
    [User_Name_Id] = LOWER(ce.[User_Name])+' ('+CONVERT(varchar,RTRIM(ce.[Site_User_Id]))+')'
    ,[Witness] = COALESCE(LOWER(ce.[Witness_Name])+' ('+CONVERT(varchar,RTRIM(ce.[Witness_Site_User_Id]))+')','')
    ,[Event_Instant] = CONVERT(datetime2(0),ce.[Event_Dttm])
...
WHERE ce.[Event_Dttm] BETWEEN @searchStartTime AND @searchStopTime
	AND ce.[Event_Type_Num] IN (1,2,4,7) --Dispense, return, restock, and return bin audit only
	AND dd.[Drug_Dose_Id] = @ddId
	--AND ce.[Unit_Id] = @ADCunit
	AND ce.[Station_Id] = @ADC
